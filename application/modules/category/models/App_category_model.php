...
class App_category_model extends CI_Model{
	...
	protected $tbl_visa_category = 'tbl_visa_category';
	protected $tbl_gallery_banner = 'tbl_gallery_banner';
	protected $tbl_visa_data = 'tbl_visa_data';
	protected $tbl_language = 'tbl_language';
	
	
	public function insertCategory($post_value){
		if(!empty($post_value)){
			$this->db->insert('tbl_visa_category', $post_value);
	...
	public function deleteCate($id){
		...
		if($query->num_rows() > 0){
			...
			$this->db->where('vcategory_id', $id);
			$this->db->delete($this->tbl_visa_data);
