...
class Contact extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($postData)){
			$rul = array( 
				...
				if ($this->form_validation->run() == TRUE && strtoupper($this->session->userdata('captchaWord')) == strtoupper($this->input->post('captchaCode')) ) {
	...
	*/
	...
	private function contactmail($post){
		...
		#SMTP
			...
			//Load email library 
			$this->load->library('email'); 
			$this->email->from($from_email, 'The Oman Visa'); 
			$this->email->to('info@theomanvisa.com');
			//$this->email->cc('sumit.makan@csipl.net'); 
			//$this->email->bcc('sumit.makan@csipl.net'); 
	...
	*/
	...
	public function requestCall(){
		...
		$phone= $post['txtPhone'];
		$email= $post['txtEmail'];
		$formsubject = $post['txtSubject'];
		$subject = "Request Callback";
		$formmessage= $post['txtMessage'];
		$to = "info@theomanvisa.com";
		$from    = $email;
		$email_id = $to;
       // $subject  = $formsubject;		
		#Insert Data
		$insert_id = $this->Contactmodel->addCallbackData($post);
			...
			$message .= '<tr  bgcolor="#ffffff">';
			$message .= '<td  valign="top" style="color:#535258; font-weight:normal; " width="100"><strong>Message:</strong></td>';
			$message .= '<td width="470" style="color:#535258;">' . @$formmessage . '</td>';
			$message .= '</tr>'; 
			$message .= '</table>';
			$message .= '</td>';
			$message .= '</tr>';
			...
			$this->load->library('email'); 
			$this->email->from($from_email, 'The Oman Visa'); 
			//$this->email->to('info@theomanvisa.com , support@theOmanvisa.com , vicky@theOmanvisa.com ');
			$this->email->to('info@theomanvisa.com ,support@theomanvisa.com');
			$this->email->subject($subject); 
			$this->email->message($message);
			$this->email->set_mailtype('html');
			if (@$this->email->send()){
				...
				$this->session->set_flashdata('neg', ' <strong>Oops!</strong> There is some error while sending the message. Please try again!');
				redirect('home');
			}		
    }
